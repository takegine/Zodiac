...
(function()	{
    ...
    for (i=0;i<hardmode;i++){

    	$("#UIstartmain").BCreateChildren("<Button id='ModePanel"+i+"' class='ModePanel' hittest='false' onactivate='OpenInfo("+(i+1)+")' />");
    	$("#ModePanel"+i).BCreateChildren("<Label class='modelabel' text='#mode"+(i+1)+"'/>");
    	$("#ModePanel"+i).BCreateChildren("<Panel id='spl"+i+"'/>");

    	for (j=0;j<10;j++){
    	$("#ModePanel"+i).BCreateChildren("<DOTAAvatarImage id='ava"+i+j+"' steamid=' ' class='userhead'/>");
