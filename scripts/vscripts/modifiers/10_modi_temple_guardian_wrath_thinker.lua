...
function modifier_temple_guardian_wrath_thinker:OnCreated( kv )
	if IsServer() then
		...
		self.blast_damage = self:GetAbility():GetSpecialValueFor( "blast_damage" ) * _G.hardmode
		
				
		self:StartIntervalThink( self.delay )

		local nFXIndex = ParticleManager:CreateParticle( "particles/units/heroes/hero_pugna/pugna_netherblast_pre_d.vpcf", PATTACH_CUSTOMORIGIN, nil )
...
function modifier_temple_guardian_wrath_thinker:OnIntervalThink()
	if IsServer() then
		local nFXIndex = ParticleManager:CreateParticle( "particles/units/heroes/hero_pugna/pugna_netherblast_bits.vpcf", PATTACH_CUSTOMORIGIN, nil )
		ParticleManager:SetParticleControl( nFXIndex, 0, self:GetParent():GetOrigin() )
