LinkLuaModifier( "modifier_my_relic_proved", "modifiers/mod_proved", LUA_MODIFIER_MOTION_NONE )

mod_proved = class({})
--------------------------------------------------------------------------------

--function mod_proved:GetEffectName()
--	return "particles/my_doom_bringer_doom_ring.vpcf"
--end

function mod_proved:IsHidden() 
	return false
end

function mod_proved:GetTexture()
    return self.ico
end

--------------------------------------------------------------------------------

function mod_proved:IsPurgable()
	return false
end

function mod_proved:RemoveOnDeath()
    return false
end

----------------------------------------
function mod_proved:DeclareFunctions()
	local funcs = 
	{
...
function mod_proved:OnCreated( kv )
    ...
    if IsServer() and not self:GetCaster():IsIllusion() then
        local nFXIndexseal = ParticleManager:CreateParticle( "particles/my_doom_bringer_doom_ring.vpcf", PATTACH_CUSTOMORIGIN, nil )
        ParticleManager:SetParticleControlEnt(nFXIndexseal,0,self:GetCaster(),PATTACH_ABSORIGIN_FOLLOW,nil,self:GetCaster():GetOrigin(),true)
        local colorlist = {
            Vector(128,0,188),
            Vector(87, 188, 255),
            Vector(255, 153, 0),
        ...
        ParticleManager:SetParticleControl(nFXIndexseal,5,colorlist[self:GetCaster().sealcolor])
        self:GetCaster().sealpfx = nFXIndexseal
    ...
    if IsClient() then
        if self:GetAbility():GetName() == "item_proved" then
            self.ico = "custom/item_proved"
        else
            self.ico = "custom/item_finalbook"
        end
    end
    self:OnWaweChange(0)
end
...
function mod_proved:OnIntervalThink()
    if IsServer() then
        local colorlist = {
            Vector(128,0,188),
            Vector(87, 188, 255),
        ...
        ParticleManager:SetParticleControl(self:GetCaster().sealpfx,5,colorlist[self:GetCaster().sealcolor])
    end
end

function mod_proved:OnRespawn(event)
    if IsServer() then
        if event.unit == self:GetCaster() then
            ParticleManager:DestroyParticle(self:GetCaster().sealpfx, false)
            local nFXIndexseal = ParticleManager:CreateParticle( "particles/my_doom_bringer_doom_ring.vpcf", PATTACH_CUSTOMORIGIN, nil )
            ParticleManager:SetParticleControlEnt(nFXIndexseal,0,self:GetCaster(),PATTACH_ABSORIGIN_FOLLOW,nil,self:GetCaster():GetOrigin(),true)
            local colorlist = {
                Vector(128,0,188),
...
----------------------------------------
function mod_proved:OnWaweChange(wave)
...
function mod_proved:GetModifierSpellLifesteal( params )
    if IsServer() then
...
function mod_proved:GetModifierPreAttack_BonusDamage( params )
	if IsServer() then
        if not self:GetCaster():IsIllusion() then
            local damag = 0
...
function mod_proved:GetModifierAttackSpeedBonus_Constant( params )
	if IsServer() then
        if not self:GetCaster():IsIllusion() then
            local damag = 0
...
function mod_proved:GetModifierPhysicalArmorBonus( params )
	if IsServer() then
        if not self:GetCaster():IsIllusion() then
            local damag = 0
...
function mod_proved:GetModifierMagicalResistanceBonus( params )
	if IsServer() then
        if not self:GetCaster():IsIllusion() then
            local damag = 0
...
function mod_proved:GetModifierBonusStats_Strength( params )
	if IsServer() then
        if not self:GetCaster():IsIllusion() then
            local damag = 0
...
function mod_proved:GetModifierBonusStats_Agility( params )
	if IsServer() then
        if not self:GetCaster():IsIllusion() then
            local damag = 0
            if self:GetCaster().lvl_item_oblivions_locket ~= nil then
...
function mod_proved:GetModifierBonusStats_Intellect( params )
	if IsServer() then
        if not self:GetCaster():IsIllusion() then
            local damag = 0
            if self:GetCaster().lvl_item_oblivions_locket ~= nil then
...
function mod_proved:GetModifierSpellAmplify_Percentage( params )
	if IsServer() then
        if not self:GetCaster():IsIllusion() then
            local damag = 0
...
---------------------------------------------------
function mod_proved:OnTooltip(params)
    local atr = CustomNetTables:GetTableValue("RelicStats_Tabel",tostring(-1-self:GetStackCount()).."0")
    if atr ~= nil then
        return atr.param
    end
...
function mod_proved:OnAbilityExecuted( params )
	if IsServer() then
        if not self:GetCaster():IsIllusion() then
...
function mod_proved:OnAttackLanded( params )
	if IsServer() then
        if not self:GetCaster():IsIllusion() then
...
function mod_proved:OnTakeDamage( params )
	if IsServer() then
