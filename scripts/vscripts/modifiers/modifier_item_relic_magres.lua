...
function modifier_item_relic_magres:IsHidden() 
	return false
end

--------------------------------------------------------------------------------

function modifier_item_relic_magres:IsPurgable()
	return false
end

function modifier_item_relic_magres:GetTexture ()
    return "custom/relic_magres"
end
...
function modifier_item_relic_magres:OnWaweChange( wawe )
	if IsServer() then
        self.wave = wawe
        local damag = 0
        if self:GetCaster().lvl_item_relic_magres ~= nil then
            if self:GetCaster().lvl_item_relic_magres >= 20 then
                damag = math.floor(self.bonus_damage_for_wawe * self.wave * 20)
            else
                damag = math.floor(self.bonus_damage_for_wawe * self.wave * self:GetCaster().lvl_item_relic_magres)
            end
            if damag ~= self:GetStackCount() then
                self:SetStackCount(damag)
            end
        else
            self:GetCaster().lvl_item_relic_magres = 1
        end
    end
end
