...
function modifier_imba_generic_talents_handler:OnAbilityFullyCast(keys)
	if (keys.ability:GetName() == "arc_warden_tempest_double") and (keys.unit == self:GetParent()) then
		local heroes =  FindUnitsInRadius(keys.unit:GetTeamNumber(),  --队伍
						keys.unit:GetAbsOrigin(), nil, FIND_UNITS_EVERYWHERE,--圆心，nil,半径(全地图)
						DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_HERO,--目标为友方队伍，英雄类
		    			DOTA_UNIT_TARGET_FLAG_OUT_OF_WORLD + DOTA_UNIT_TARGET_FLAG_INVULNERABLE, --目标包括隐身和无敌单位
		    			FIND_ANY_ORDER, false)--随机
