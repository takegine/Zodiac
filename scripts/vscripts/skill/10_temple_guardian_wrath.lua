my_lava_boom = class({})
LinkLuaModifier( "modifier_temple_guardian_wrath_thinker", "modifiers/10_modi_temple_guardian_wrath_thinker", LUA_MODIFIER_MOTION_NONE )
...
function my_lava_boom:OnAbilityPhaseStart()
	if IsServer() then
		self.channel_duration = self:GetSpecialValueFor( "channel_duration" )+_G.hardmode *2 - 2
		local fImmuneDuration = self.channel_duration + self:GetCastPoint()
        
        self.nPreviewFX = ParticleManager:CreateParticle( "particles/econ/items/earthshaker/earthshaker_arcana/earthshaker_arcana_death_body.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetCaster() )
...
function my_lava_boom:OnSpellStart()
	if IsServer() then	
        ...
        towers = Entities:FindAllByModel("models/props_structures/tower_upgrade/tower_upgrade.vmdl")--按照模型查找
        if #towers > 0 then
        local wow = 1
